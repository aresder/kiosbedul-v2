...
const ProductGamesComponent = ({ gameList }: { gameList: GamesList[] }) => {
  return (
    <div className="w-full text-white mt-16">
      <div className="flex gap-4 px-4">
        <span className="border-r-4 border-orange-500"></span>
        <h1 className="text-base sm:text-2xl text-white uppercase py-2 font-semibold">
          🎮 games
        </h1>
      </div>
      <div className="mt-8 flex justify-center items-center md:mx-8">
        <div className="w-full grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-4">
          {gameList.map((game) => (
            <Link
              href={""}
...
export default ProductGamesComponent;
